...
const HeaderEmployee = ({ userRole, userId, headerBackgroundColor, descriptors }: { headerBackgroundColor?: string, descriptors: any }) => {
    ...
    const [activities, setActivities] = useState(['LoanBookEmployee', "CashFlowEmployee"])
    const [firm, setFirm] = useState({})

    useEffect(() => {
        const fetchData = async () => {
            try {
                const firmData = await getEmployeeFirmData(userId);
                setFirm(firmData)
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('tabPress', (e) => {
            if (isHomeFocused) {
                ...
                if (currentCategory !== 'LoanBookEmployee') {
                    navigation.navigate('LoanBookEmployee');
                    e.preventDefault();
                }
            }
        });
